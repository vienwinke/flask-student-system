    ...
    try:
        with conn.cursor() as cur:
            cur.execute('REPLACE INTO subject_weight(subject_name, weight_ratio) VALUES(%s, %s)',
                        (data['subject'], float(data['weight'])))
            conn.commit()
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'error': str(e)}), 400
                ...
                for row_idx, row in enumerate(data_rows, 2):
                    ...
                    if has_sid and has_name and vals[col_sid] and vals[col_name]:
                        ...
                        try:
                            ...
                            result['errors'].append('\u7b2c%d\u884c\u5b66\u751f\u5bfc\u5165\u5931\u8d25: %s' % (row_idx, str(ex)))

                    if has_sid and has_subject and vals[col_sid] and vals[col_subject]:
                        sid = vals[col_sid]
                        subject = vals[col_subject]
                        usual = int(float(vals[col_usual])) if col_usual >= 0 and vals[col_usual] else 0
                        end = int(float(vals[col_end])) if col_end >= 0 and vals[col_end] else 0
                        ...
                        try:
                            ...
                            result['errors'].append('\u7b2c%d\u884c\u6210\u7ee9\u5bfc\u5165\u5931\u8d25: %s' % (row_idx, str(ex)))
        finally:
            conn.close()
    finally:
        os.unlink(tmp.name)
